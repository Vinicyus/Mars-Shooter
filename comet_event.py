...
#créer uine classe pour gérer cet evenement
class CometFallEvent:

	# lors du chargement -> créer un compteur
	def __init__(self, game):
		self.percent = 0
		self.pecent_speed = 10
		self.game = game
	...
	def add_percent(self):
		self.percent += self.pecent_speed / 100
	...
	def meteor_fall(self):
		# boucle pour le valeurs entre 1 es 10
		for i in range(1, 10):
			# apparaitre 1 premiere boule de feu
			self.all_comets.add(Comet(self))

	def attempt_fall(self):
		# la jauge d'evement est totalment chargé
